...
class OpenLoopVelocityParams:
    u_constant: float


def load_open_loop_velocity_params_csv(path: str | Path) -> list[OpenLoopVelocityParams]:
    ...
    * **3 rows** – one row per electrode (El1, El2, El3 in order).
    ...
    if len(df) == 1:
        p = OpenLoopVelocityParams(u_constant=float(df["u_constant"].iloc[0]))
        return [p, p, p]

    if len(df) >= 3:
        return [OpenLoopVelocityParams(u_constant=float(df["u_constant"].iloc[i])) for i in range(3)]

    raise ValueError("Open-loop params CSV must have either 1 row (broadcast) or at least 3 rows")


class OpenLoopVelocityController:
    """Open-loop controller: outputs ``u_constant * reference``.

    Useful as a feedforward baseline or for open-loop step tests.
    """

    def __init__(self, params: OpenLoopVelocityParams, dt: float) -> None:
        self.params = params
        self.dt = float(dt)
    ...
    def step(self, reference: float, y_pred: float, u_prev: float) -> tuple[float, float]:
        ...
        u_des = self.params.u_constant * float(reference) + float(u_prev)
        return u_des, e