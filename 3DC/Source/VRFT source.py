...
def M_cont_to_disc(tau,t,q,Ts):
    ...
    den_coeff=list(reversed(den_coeff)) # Polypow takes coefficients in ascending order, scipy transfer function takes them in descending order.
...
# This function must be fitted manually, and must be a rational fraction where the numerator and denominator are polynomials.
...
def GetFilterCoeff(num,den,lp_num,lp_den,phi_num,phi_den):
    ...
    def subConstructRational(num,den):
        # Constructs a rational function object for internal use
    ...
    F_aux = (1 - M) * W * Phi_Sqrt_inv  # Auxiliary filter to avoid having to calculate r_v directly, which circumvents the scipy time-delay issue.
